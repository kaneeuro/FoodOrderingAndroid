...
public class ServeurResumeCommandeActivity extends AppCompatActivity {
    ...
    public void saveCommandeArticle() {
        Commande commande = new Commande(null, null, Long.valueOf(Constantes.randomString()), 1, null, LoginActivity.compte.getEmploye().getIdEmploye(), Long.valueOf(MainActivity.numeroTable));
        retrofitService.saveCommande(commande).enqueue(new Callback<Commande>() {
            @Override
            public void onResponse(Call<Commande> call, Response<Commande> response) {
                if (response.isSuccessful()){
                    commandeServeur = response.body();
                    Log.e("COMMANDE",commandeServeur.getIdCommande()+"-"+commandeServeur.getNumero()+"-"+commandeServeur.getServeur().getNom()+"-"+commandeServeur.getTable().getIdTable());
                    ...
                    for (CommandeArticleTemporaire cat:articles) {
                        ...
                        commandeArticle=new CommandeArticle(cat.getArticle().getPrix(), cat.getArticle().getPrix()*quantiteCommande, quantiteCommande, cat.getArticle().getIdArticle(), commandeServeur.getIdCommande());
                        retrofitService.saveCommandeArticle(commandeArticle).enqueue(new Callback<CommandeArticle>() {
                            @Override
                            public void onResponse(Call<CommandeArticle> call, Response<CommandeArticle> response) {
                                if (response.isSuccessful()){
                                    //commandeServeur = response.body().getCommande();
                                    Log.e("COMMANDE_ARTICLE",response.body().getIdCommandeArticle().toString());
                                }
                            }
                            ...
                            public void onFailure(Call<CommandeArticle> call, Throwable t) {
                                serverDialog(contextView);
                                Log.e("ERREUR_COMMANDE_ARTICLE",t.getMessage());
                            }
                        });
                    }
            ...
            public void onFailure(Call<Commande> call, Throwable t) {
                serverDialog(contextView);
                Log.e("ERREURCOMMANDE",t.getMessage());
            }
        });
    ...
    /*public void saveCommande() {
        Commande commande = new Commande(null, null, Long.valueOf(Constantes.randomString()), 1, null, LoginActivity.compte.getEmploye().getIdEmploye(), new Tables(MainActivity.numeroTable).getIdTable());
        retrofitService.saveCommande(commande).enqueue(new Callback<Commande>() {
                @Override
                public void onResponse(Call<Commande> call, Response<Commande> response) {
                    if (response.isSuccessful()){
    ...
    private void deleteAllArticlesTemporaires(){
        retrofitService.deleteAllArticlesTemporairesByEmploye(LoginActivity.compte.getEmploye().getIdEmploye()).enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                if (response.isSuccessful()){
                    Log.e("DELETED",response.body());
                }
            }
